...
## Opening the Annotation View [#open-annotation-view]
...
## Linking with Selected Literature [#linking-with-selected-literature]
...
## Follow Zotero reader [#follow-zotero-reader]
...
<Steps>
### Enable Backgound Connection in Obsidian [#enable-backgound-connection-in-obsidian]
...
### Enable Backgound Connection in Zotero [#enable-backgound-connection-in-zotero]

In Zotero, Go to `Settings` > `ZotLit` > `Notify` and toggle `Enable Notify`

![Backgound Connect in Zotero](/img/bg-connect-zotero.png)

Make sure that the port number matches the one in Obsidian.

### Enable `Follow Zotero Reader` Mode [#enable-follow-zotero-reader-mode]
...
## Import Annotation by Drag and Drop [#import-annotation-by-drag-and-drop]
